...
void VRMenu::Frame(
    ...
    }else if( Flags &VRMENU_FLAG_TRACK_GAZE_VERTICAL){
        MenuPose = CalcMenuPositionOnVertical(centerViewMatrix);
    }
...
Posef VRMenu::CalcMenuPositionOnHorizon(Matrix4f const& viewMatrix) const {
    ...
    Vector3f const up(0.0f, 1.0f, 0.f);
        up.Normalized();
    ...
    Posef VRMenu::CalcMenuPositionOnVertical(Matrix4f const& viewMatrix) const {
        const Vector3f viewPos(GetViewMatrixPosition(viewMatrix));
        const Vector3f viewFwd(GetViewMatrixForward(viewMatrix));
        ...
        up.Normalized();
        Vector3f const look(1.0f, 0.0f, 0.0f);

        look.Normalized();
        float dot = viewFwd.Dot(look);
        Vector3f horizontalFwd =
                (dot < -0.99999f || dot > 0.99999f) ? Vector3f(0.0f, 1.0f, 0.0f) :
                viewFwd - (look * dot);
        horizontalFwd.Normalize();


        Vector3f horizontal = horizontalFwd.Cross(up);

        Vector3f horizontal_up = horizontal.Cross(horizontalFwd);
        horizontal_up.Normalize();


       // Vector3f(0)
        Vector3f at = horizontalFwd* MenuDistance;
        Vector3f eye = horizontal* MenuX + horizontal_up * MenuY;

        Matrix4f horizontalViewMatrix = Matrix4f::LookAtRH(eye, at, up);

   //     horizontalViewMatrix =horizontalViewMatrix* Matrix4f::RotationX(0.5);
        ...
        Quatf fullRotation = /*rotation * */ viewRot;
        fullRotation.Normalize();
// + horizontalFwd * MenuDistance+ horizontal_up *0.5
        Vector3f position(viewPos +(at -eye ));
