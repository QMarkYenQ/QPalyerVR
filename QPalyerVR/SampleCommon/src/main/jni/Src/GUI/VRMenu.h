...
enum eVRMenuFlags {
    ...
    // place the menu directly in front of the user's view on the horizon plane each frame. The user
    // will sill be able to gaze track vertically.
    VRMENU_FLAG_TRACK_GAZE_VERTICAL,
...
class VRMenu {
   public:
    ...
    void SetMenuDistance(float distance) {
    ...
    OVR::Posef CalcMenuPositionOnVertical(OVR::Matrix4f const& viewMatrix) const;

    float GetMenuDistance() const {
        return MenuDistance;
    }
    void SetMenuDistance(OVR::Vector3f  menuDistance) {
        MenuDistance = menuDistance[2];

        MenuX = menuDistance[0];

        MenuY = menuDistance[1];
    }
   ...
   private:
    ...
    float MenuX; // distance from eyes
    float MenuY; // distance from eyes
